...
int main()
{
    ...
    part_stat partions;
    status st = get_part_stat(&partions);
    if(st!=ok){
    	return -1;
    }
    ...
    while(1)
    {
	    ...
	    case '\t': // переход на следующую панель
			...
			if(pl==3) { pl=0; } else { pl++; } // if last panel cnt switch it to 0
			draw_title(pl);
			break;
	    case KEY_DOWN: //
	    {
    		...
    		case 2:
    			{
    				...
    				for(i=0;i<mlist.num_nodes;i++) {
    					wmove(top->win,i+2,1);
    					char str[256];
    					uint total = mlist.mounts[i].blocks;
    					uint ava = mlist.mounts[i].blocks_available;
    					uint use = mlist.mounts[i].use;
